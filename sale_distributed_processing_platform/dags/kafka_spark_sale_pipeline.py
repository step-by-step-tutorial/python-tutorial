...
def process_sale_event_batch(dataframe: DataFrame, batch_id: int, ingestion_time: datetime) -> None:
    ...
    try:
        ...
        datalake_sale_service.append(dataframe=raw_output_dataframe, bucket_name=ec.DATALAKE_BUCKET_NAME, path=raw_sale_data_path)

        logger.info("Appending cleaned sale data to %s", cleaned_sale_data_path)
        datalake_sale_service.append(dataframe=cleaned_output_dataframe, bucket_name=ec.DATALAKE_BUCKET_NAME, path=cleaned_sale_data_path)

        logger.info("Appending enriched sale data to %s", enriched_sale_data_path)
        datalake_sale_service.append(dataframe=enriched_output_dataframe, bucket_name=ec.DATALAKE_BUCKET_NAME, path=enriched_sale_data_path)
