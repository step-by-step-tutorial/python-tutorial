...
class AuditService:
    ...
    def emit(self, event: AuditEvent) -> None:
        logger.info("Emitting audit event: event_id=%s type=%s status=%s pipeline=%s task=%s", event.event_id, event.event_type.value, event.status.value, event.pipeline_name, event.task_name)
