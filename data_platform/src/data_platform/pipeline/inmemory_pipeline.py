...
class InmemoryPipeline(BatchPipeline):
    ...
    def __init__(self, ds: Dataset) -> None:
        ...
        self.database_repository = DatabaseRepository(self.dataset.get_endpoint_by_role(EndpointRole.DATABASE, DatabaseEndpoint))
        self.data_lake_repository = DataLakeRepository(self.dataset.get_endpoint_by_role(EndpointRole.DATA_LAKE, DataLakeEndpoint))
        self.data_warehouse_repository = DataWarehouseRepository(
            self.dataset.get_endpoint_by_role(EndpointRole.DATA_WAREHOUSE, DataWarehouseEndpoint))
        self.raw_data_ingestor = ingestor_registry.get_item(
            self.dataset.get_endpoint_by_role(EndpointRole.FILE_CSV, FileEndpoint).name
        )
        self.populators = (
            RepositoryDataPopulator(self.download_enriched_data, self.database_repository.truncate_and_populate_from_memory),
            RepositoryDataPopulator(self.download_enriched_data, self.data_warehouse_repository.truncate_and_populate_from_memory),
        )
        self.analyzers = (
