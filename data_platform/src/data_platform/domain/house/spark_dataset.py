...
from data_platform.spark_cleaners.cleaner_impl import BooleanColumnCleaner, DropDuplicatesCleaner, NumericColumnCleaner, StripColumnCleaner, ToDatetimeCleaner
...
spark_house_dataset = Dataset(
    ...
    flow=SparkPipelineFlow(
        ...
        validators=SparkValidatorChain((RequiredColumnsValidator(attribute.columns), NotNullValidator(attribute.property_id), NotNullValidator(attribute.area_sqm), NotNullValidator(attribute.total_price), PositiveValidator(attribute.area_sqm), PositiveValidator(attribute.total_price))),
        enrichers=SparkEnricherChain(),
        exposers=(DataExposer((SparkDatabaseRepository(house_spark_endpoints[Key.HOUSE_DATABASE]).overwrite,)), DataExposer((SparkWarehouseRepository(house_spark_endpoints[Key.HOUSE_WAREHOUSE]).overwrite,))),
        analyzers=SparkAnalyzerChain((
            GroupAggregateAnalyzer("property_count_by_city", AggregateSpecification(attribute.city, attribute.property_id, "count", "property_count")),
            GroupAggregateAnalyzer("average_total_price_by_city", AggregateSpecification(attribute.city, attribute.total_price, "avg", "average_total_price")),
            GroupAggregateAnalyzer("average_price_per_square_meter_by_property_type", AggregateSpecification(attribute.property_type, attribute.price_per_sqm, "avg", "average_price_per_sqm")),
            GroupAggregateAnalyzer("average_living_area_by_property_type", AggregateSpecification(attribute.property_type, attribute.living_area_sqm, "avg", "average_living_area_sqm")),
            GroupAggregateAnalyzer("average_energy_consumption_by_city", AggregateSpecification(attribute.city, attribute.annual_energy_consumption_kwh, "avg", "average_energy_consumption_kwh")),
        )),
    ),
)
