...
from data_platform.spark_analyzers.analyzer_impl import GroupAggregateAnalyzer
from data_platform.spark_cleaners.cleaner_chain import SparkCleanerChain
from data_platform.spark_cleaners.cleaner_impl import DropDuplicatesCleaner, NumericColumnCleaner, StripColumnCleaner, ToDatetimeCleaner
from data_platform.spark_enrichers.enricher_chain import SparkEnricherChain
from data_platform.spark_enrichers.enricher_impl import CopyColumnEnricher, DatetimePartEnricher, PercentageEnricher
...
spark_online_shopping_dataset = Dataset(
    ...
    dataframe=DataFrameModel(
        ...
        required_columns=frozenset({attribute.order_id, attribute.order_date, attribute.sales_channel, attribute.country, attribute.product_name, attribute.unit_price, attribute.quantity, attribute.total_amount}),
    ),
    ...
    flow=SparkPipelineFlow(
        ...
        exposers=(DataExposer((SparkDatabaseRepository(online_shopping_spark_endpoints[Key.ONLINE_SHOPPING_DATABASE]).overwrite, SparkWarehouseRepository(online_shopping_spark_endpoints[Key.ONLINE_SHOPPING_WAREHOUSE]).overwrite)),),
        analyzers=SparkAnalyzerChain((
            GroupAggregateAnalyzer("revenue_by_country", AggregateSpecification(attribute.country, attribute.net_revenue, "sum", attribute.revenue)),
            GroupAggregateAnalyzer("revenue_by_sales_channel", AggregateSpecification(attribute.sales_channel, attribute.net_revenue, "sum", attribute.revenue)),
            GroupAggregateAnalyzer("order_count_by_country", AggregateSpecification(attribute.country, attribute.order_id, "count", "order_count")),
            GroupAggregateAnalyzer("revenue_by_category", AggregateSpecification(attribute.category, attribute.net_revenue, "sum", attribute.revenue)),
            GroupAggregateAnalyzer("average_order_value_by_sales_channel", AggregateSpecification(attribute.sales_channel, attribute.total_amount, "avg", "average_order_value")),
            GroupAggregateAnalyzer("average_delivery_days_by_shipping_method", AggregateSpecification(attribute.shipping_method, attribute.delivery_days, "avg", "average_delivery_days")),
            GroupAggregateAnalyzer("order_count_by_fulfillment_status", AggregateSpecification(attribute.fulfillment_status, attribute.order_id, "count", "order_count")),
        )),
    ),
)
