...
_DOUBLE_COLUMNS = {attribute.unit_price, attribute.subtotal, attribute.discount_percent, attribute.shipping_cost, attribute.tax_amount, attribute.total_amount, attribute.discount_amount, attribute.net_revenue, attribute.revenue}
_TIMESTAMP_COLUMNS = {attribute.order_date, attribute.estimated_delivery_date}
_DERIVED_COLUMNS = {attribute.discount_amount, attribute.net_revenue, attribute.year, attribute.month, attribute.revenue}


ONLINE_SHOPPING_SCHEMA = StructType([
    *[
        StructField(column, IntegerType() if column in _INTEGER_COLUMNS else DoubleType() if column in _DOUBLE_COLUMNS else TimestampType() if column in _TIMESTAMP_COLUMNS else StringType(), nullable=True)
