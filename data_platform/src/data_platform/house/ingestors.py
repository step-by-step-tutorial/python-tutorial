...
def register_house_ingestors() -> None:
    if not ingestor_registry.contains(Key.HOUSE_FILE_CSV):
        house_file_csv_ingestor = CsvFileIngestor(HOUSE_DATASET.get_endpoint(Key.HOUSE_FILE_CSV, FileEndpoint))
        ingestor_registry.register(Key.HOUSE_FILE_CSV, house_file_csv_ingestor)
    if not ingestor_registry.contains(Key.HOUSE_KAFKA_LISTENER):
        house_kafka_listener_ingestor = KafkaIngestor(
            HOUSE_DATASET.get_endpoint(Key.HOUSE_KAFKA_LISTENER, MessagingEndpoint)
        )
        ingestor_registry.register(Key.HOUSE_KAFKA_LISTENER, house_kafka_listener_ingestor)
    ...
    if not ingestor_registry.contains(Key.HOUSE_DATALAKE):
        house_data_lake_ingestor = DataLakeIngestor(HOUSE_DATASET.get_endpoint(Key.HOUSE_DATALAKE, DataLakeEndpoint))
        ingestor_registry.register(Key.HOUSE_DATALAKE, house_data_lake_ingestor)
    ...
    if not ingestor_registry.contains(Key.HOUSE_DATAWAREHOUSE):
        house_data_warehouse_ingestor = DataWarehouseIngestor(
            HOUSE_DATASET.get_endpoint(Key.HOUSE_DATAWAREHOUSE, DataWarehouseEndpoint)
        )
        ingestor_registry.register(Key.HOUSE_DATAWAREHOUSE, house_data_warehouse_ingestor)
